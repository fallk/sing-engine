...
void CBasePlayer::StartObserver( Vector vecPosition, Vector vecViewAngle )
{
	...
	fprintf(f, "v_angle(start observer 1): %f\n", pev->v_angle);
	pev->angles = pev->v_angle = vecViewAngle;
	fprintf(f, "v_angle(start observer 2): %f\n", pev->v_angle);
	fclose(f);
	pev->fixangle = TRUE;
	pev->solid = SOLID_NOT;
	pev->takedamage = DAMAGE_NO;
...
void CBasePlayer::PlayerUse ( void )
{
	...
	float flDot;
	FILE * f = fopen("test.log", "a");
	fprintf(f, "v_angle(player use1): %f\n", pev->v_angle);
	UTIL_MakeVectors ( pev->v_angle );// so we know which way we are facing
	fprintf(f, "v_angle(player use 2): %f\n", pev->v_angle);
	fclose(f);
...
void CBasePlayer::PreThink(void)
{
	...
	FILE * f = fopen("test.log", "a");
	fprintf(f, "v_angle(prethink): %f\n", pev->v_angle[0]);
	UTIL_MakeVectors(pev->v_angle);             // is this still used?
	fprintf(f, "v_angle(prethink 2): %f\n", pev->v_angle[0]);
	fclose(f);
	ItemPreFrame( );
	WaterMove();
...
int CBasePlayer::Restore( CRestore &restore )
{
	...
	FILE * f = fopen("test.log", "a");
	fprintf(f, "v_angle(restore): %f\n", pev->v_angle[0]);
	pev->v_angle.z = 0;	// Clear out roll
	pev->angles = pev->v_angle;
	fprintf(f, "v_angle(restore 2): %f\n", pev->v_angle[0]);
	fclose(f);
