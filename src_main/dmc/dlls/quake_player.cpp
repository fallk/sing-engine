...
void CBasePlayer::UpdateStatusBar()
{
	...
	fprintf(f, "v_angle(UpdateStatusBar): %f %f %f\n", pev->v_angle[0], pev->v_angle[1], pev->v_angle[2]);
	fprintf(f, "1 %f %f %f\n",gpGlobals->v_forward[0], gpGlobals->v_forward[1], gpGlobals->v_forward[2]);
	UTIL_MakeVectors( pev->v_angle + pev->punchangle );
	fprintf(f, "v_angle(UpdateStatusBar): %f %f %f\n", pev->v_angle[0], pev->v_angle[1], pev->v_angle[2]);
	fprintf(f, "1 %f %f %f\n",gpGlobals->v_forward[0], gpGlobals->v_forward[1], gpGlobals->v_forward[2]);
	Vector vecSrc = EyePosition();
	Vector vecEnd = vecSrc + (gpGlobals->v_forward * MAX_ID_RANGE);
	fprintf(f, " %f %f %f %f %f %f\n %f %f %f\n", vecSrc.x, vecSrc.y, vecSrc.z, vecEnd.x, vecEnd.y, vecEnd.z, gpGlobals->v_forward[0], gpGlobals->v_forward[1], gpGlobals->v_forward[2]);
	fclose(f);
